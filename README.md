...
> 当前正式版本：`1.1.3`。用户不需要提前把 CSV/XLSX 改成 FulfillLens 标准列名：保留“自动识别（推荐）”，上传文件后点击“一键整理并分析”，系统会综合表头、值画像、列间关系和数据契约处理高置信映射、安全内部 ID、未知状态和非分析列；只有会改变业务含义的关键歧义才要求一次确认。Cloudflare 在线版在浏览器内解析、校验并分析自主文件，原始文件和标准化行不上传到 Worker。详见[项目状态](#项目状态与已知限制)。
...
| 行动建议   | 查看专业行动方案和管理层简报，按证据与优先级安排核查    | 两种视图共享同一确定性事实             |
...
在线自主导入与公开合成分析仍是两条独立路径：自主数据的指标、诊断、行动建议和报告由浏览器本地确定性引擎生成；自主数据的 What-if 方案管理尚未接入浏览器引擎，需使用关联订单表的本地/Docker 版。公开合成自建方案只保存在 Worker 当前运行期，不是持久业务存储。部署配置在 `wrangler.jsonc`；运行时不包含外部生成式推理绑定、探针接口或模型凭据，Cloudflare 发布凭据只用于部署过程且不进入浏览器或仓库。
...
阶段 0–12、v1.0.0 正式发布及后续稳定迭代已完成。v1.1.3 在不改变数据契约、指标公式或浏览器存储键的前提下，拒绝带显式时区的不存在日期，并安全丢弃损坏的旧分析会话，避免错误时间进入 KPI 或旧缓存导致页面崩溃。项目自带 CSV/XLSX 仍可重新上传后直接进入分析；用户文件不会自动混入演示/兼容样例，切换文件会重新计算总览、诊断、建议和报告。当前证据：
...
详见[路线图](docs/ROADMAP.md)、[最终验收记录](docs/RELEASE_ACCEPTANCE.md)、[兼容性验证报告](docs/COMPATIBILITY_VALIDATION.md)、[v1.0.0 发布说明](docs/releases/v1.0.0.md)、[v1.1.0 发布说明](docs/releases/v1.1.0.md)、[v1.1.1 发布说明](docs/releases/v1.1.1.md)、[v1.1.2 发布说明](docs/releases/v1.1.2.md)和[v1.1.3 发布说明](docs/releases/v1.1.3.md)。
...
- 运行时不集成外部生成式推理 API；指标、诊断、建议和报告均使用仓库内确定性逻辑；

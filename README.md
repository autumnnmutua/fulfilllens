...
> 当前正式版本：`1.0.1`。自主上传默认使用“自动识别（推荐）”，系统综合表头、值画像、列间关系和数据契约：高置信度字段直接应用，中等置信度建议一次批量确认，非必要列一次批量忽略。Cloudflare 在线版在浏览器内解析、校验并分析自主 CSV/XLSX，原始文件和标准化行不上传到 Worker。详见[项目状态](#项目状态与已知限制)。
...
要导入自己的文件，保留默认的“自动识别（推荐）”并点击“自主上传文件”即可。系统会根据列名、内容特征和重复关系判断“订单数据 / 仓库作业数据 / 物流轨迹数据”；高置信度映射自动完成，普通用户通常只需点击一次“全部采用推荐映射”（如有）和一次“一键忽略非必要字段”，再点击“开始分析”。完整字段表位于折叠的“高级字段设置”中，不要求先理解内部 Schema。
...
阶段 0–12、v1.0.0 正式发布和 v1.0.1 新手导入优化已完成。当前证据：

- 前端 64 项、Cloudflare Worker 14 项、后端与契约 234 项测试；
- 1 万/5 万订单性能基准；
...
详见[路线图](docs/ROADMAP.md)、[最终验收记录](docs/RELEASE_ACCEPTANCE.md)、[兼容性验证报告](docs/COMPATIBILITY_VALIDATION.md)、[v1.0.0 发布说明](docs/releases/v1.0.0.md)和[v1.0.1 发布说明](docs/releases/v1.0.1.md)。

...
> 版本状态：`1.0.0-rc.5` 将公开品牌统一为 FulfillLens，并继续强化现实文件兼容：字段建议综合规范化表头、共享业务别名和有限值特征；可一键忽略经保守规则确认的非分析列，并可撤销。必填/关键候选不会被批量忽略。Cloudflare 在线版仍在浏览器本地解析和校验自主 CSV/XLSX，原始文件不上传到 Worker。详见[项目状态](#项目状态与已知限制)。

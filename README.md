...
> 当前正式版本：`1.0.0`。现实文件兼容采用规范化表头、共享业务别名和值画像；支持一键应用高置信度建议和可撤销的安全批量忽略，必填/关键候选始终受保护。Cloudflare 在线版在浏览器内解析、校验并分析自主 CSV/XLSX，原始文件和标准化行不上传到 Worker。详见[项目状态](#项目状态与已知限制)。
...
阶段 0–12 和正式发布修复已完成。当前证据：

- 前端 50 项、Cloudflare Worker 14 项、后端与契约 234 项测试；
- 1 万/5 万订单性能基准；
...
详见[路线图](docs/ROADMAP.md)、[最终验收记录](docs/RELEASE_ACCEPTANCE.md)、[兼容性验证报告](docs/COMPATIBILITY_VALIDATION.md)和[v1.0.0 发布说明](docs/releases/v1.0.0.md)。

...
async function preview(request: ReportRequest): Promise<ReportDocument> {
  ...
  const sections: ReportSection[] = request.sections.map((code) => {
    ...
    if (code === "data_quality") {
      data = {
        ...
        raw_row_count: overview.context.raw_row_count,
        valid_row_count: overview.context.valid_row_count,
        event_count: overview.context.event_count,
        unique_shipment_count: overview.context.unique_shipment_count,
        unique_order_count: overview.context.unique_order_count,
        analysis_entity_label: overview.context.analysis_entity_label,
  ...
  return {
    header: {
      ...
      report_version: "browser-report-v1.1.1",
      renderer_version: "browser-renderer-v1.1.1",
    ...
    reading_guide: [],
    contract_version: "browser-report-v1.1.1",
  };
}

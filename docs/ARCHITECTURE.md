...
FulfillLens CN 采用本地优先的模块化单体架构，在一台普通电脑上完成数据导入、清洗、分析、诊断、模拟和导出。架构首先保证：
...
1. 用户在三类数据中选择一种，点击“自主上传文件”或在第 2 步拖拽 `.csv` / `.xlsx`；
2. 浏览器检查扩展名、MIME、文件签名、10 MiB 上限和 XLSX 解压安全限制；
3. 浏览器在内存中解析编码/工作表、字段、Excel 日期和原始标识，不执行宏、公式、脚本或外部链接；
4. 同一字段目录、Schema 和状态代码文件驱动字段建议与质量校验，用户可返回修改映射；
5. 仅在最新校验可确认时写入 IndexedDB，随后释放原始 `File` 引用；取消时直接释放；
6. 设置页列出并删除浏览器本地数据。原始文件和标准化行都不发送到 Cloudflare；
7. 浏览器自有数据尚未接入 Worker 指标/诊断/模拟/报告，公开合成案例继续使用独立 Worker 数据流，二者不伪装成同一数据上下文。

可选 Workers AI 连接是独立旁路：状态接口只返回脱敏配置；探针不读取上述 1–12 步的数据，
只发送版本化固定合成短句。未来若评估任何模型辅助能力，必须另立隐私、解释性和人工确认
契约，不能复用探针接口绕过数据边界。
...
| 领域层 | 订单级判定、节点区间、规则证据、模拟变换        | 纯函数优先，输入输出可测试                 |
| 展示层 | 指标响应、图表模型、报告模型                    | 必含版本、样本、覆盖率和警告               |
...
当前架构仍以本机文件持久化为准。`apps/cloudflare-worker/` 提供不接收业务数据的在线预览适配层：前端使用 Workers Static Assets，Worker 提供健康、版本、空数据集和 Workers AI 固定合成探针；其余数据 API 返回可解释的 501，不用静态结果伪装完整后端。未来完整云模式的控制元数据预期迁到 D1，对象预期迁到 R2；Python Workers、Containers、DuckDB 和大文件处理必须先做兼容/性能与费用 PoC。部署后的 Workers AI 使用原生 `AI` binding，并保持在指标与诊断旁路。详见 [Cloudflare 部署说明](CLOUDFLARE_DEPLOYMENT.md) 和

...
- 运行时不集成外部生成式推理 API、模型绑定或探针；指标、诊断、建议和报告均由确定性模块生成；
- Cloudflare 在线自主导入使用浏览器本地引擎，原始文件不进入 Worker；公开合成分析仍走同源 Worker API；
- v1.1.0 已实现安全导入、DuckDB/SQLite 与浏览器 IndexedDB 数据集存储、统一筛选仪表盘、版本化诊断、建议、报告和可复现 What-if；Cloudflare 自主数据的 What-if 仍明确保持不可用。
...
- 确认自主文件时创建单一分析会话，记录来源和内容指纹，并清除旧 Demo/兼容样例选择；总览、诊断、建议和报告只消费该会话；
...
8. 确认后的浏览器自有数据由纯 TypeScript 本地引擎计算指标、诊断、行动建议和报告；这些模块共享 `analysis_fingerprint`，切换文件强制重算；公开合成案例继续使用 Worker 数据流。当前浏览器自有数据不支持 What-if，两个上下文不混用；
9. 用户主动组合订单与事件时计算关联订单、孤立事件和关联率；关联率为 0 时拒绝组合，避免两个无关数据源产生看似可信的结果；
10. 行动建议先从确定性的指标与诊断生成共享 facts，再渲染专业行动方案和管理层简报；原始行不会为生成建议而发送给 AI。

当前运行时没有外部生成式推理旁路，也不保存模型凭据。建议表达直接消费版本化、最小化的 recommendation facts，并由确定性模板生成，因此外部服务不可用不会影响核心分析。
...
当前架构仍以本机文件持久化为准。`apps/cloudflare-worker/` 承载前端静态资源和公开合成案例 API；用户自主文件则由浏览器本地解析、校验并保存在 IndexedDB，再由浏览器本地引擎提供指标、诊断、建议和报告。Worker 不接收用户原始文件或标准化行，浏览器自有数据的 What-if 尚未实现。未来完整云模式的控制元数据预期迁到 D1，对象预期迁到 R2；Python Workers、Containers、DuckDB 和大文件处理必须先做兼容/性能与费用 PoC。当前部署不声明外部生成式推理 binding，也不提供模型调用路由。详见 [Cloudflare 部署说明](CLOUDFLARE_DEPLOYMENT.md) 和
[ADR 0008](adr/0008-local-and-cloudflare-runtime-boundary.md)。

## 8.2 行动建议引擎

建议引擎输入是版本化指标与诊断结果，不读取未标准化源字段。它按影响订单数、偏差程度、频率、覆盖范围和数据可信度确定高/中/观察优先级，并输出稳定 `fact_id`、数据证据、建议动作、KPI、目标方向、风险和验证步骤。专业行动方案与管理层简报只负责对同一 facts 做不同层次的确定性表达；指标不可计算时生成数据补充建议，不用零值或外部文本填空。

...
7. 确认后的浏览器自有数据由纯 TypeScript 本地引擎计算指标、诊断、行动建议和报告；公开合成案例继续使用 Worker 数据流。当前浏览器自有数据不支持 What-if，两个上下文不混用；
8. 行动建议先从确定性的指标与诊断生成共享 facts，再渲染专业行动方案和管理层简报；原始行不会为生成建议而发送给 AI。

...
const outputPath = process.env.FL_AUDIT_OUTPUT || "docs/chromium-audit-results.json";
const axePath =
  process.env.FL_AXE_PATH ||
  path.resolve(__dirname, "..", "node_modules", "axe-core", "axe.min.js");
const routes = ["/", "/import", "/analytics", "/diagnostics", "/scenarios", "/cases", "/reports", "/settings"];
const viewports = [
...
function accessibleProblems() {
  ...
  const interactive = [...document.querySelectorAll("button,a[href],input,select,textarea,[role=button],[role=link],[role=combobox]")]
  ...
  return {
    ...
    unnamedInteractive: interactive.filter((element) => !name(element)).map((element) => element.outerHTML.slice(0, 160)),
    duplicateIds: [...new Set(duplicateIds)],
    tablesWithoutCaption: [...document.querySelectorAll("table")]
      .filter(
        (table) =>
      ...
      .length,
    imagesWithoutText: [...document.querySelectorAll("img,[role=img]")]
